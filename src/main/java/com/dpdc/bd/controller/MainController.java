...
import com.dpdc.bd.dao.MainDao;
...
import com.dpdc.bd.model.WalkSeqModel;
import com.dpdc.bd.model.MRSEntryFormModel;
...
public class MainController {
	...
	public String mrs(Model model, @CookieValue(value = "user_name", defaultValue = "") String user_name) {
		...
		//user_name="DIPU";
		  if (user_name.equals("")) { return "redirect:/"; }
	...
	public String TableData(MRSEntryModel mrsEntryModel, RedirectAttributes redirectAttributes,
			@CookieValue(value = "user_name", defaultValue = "") String user_name) {
		ArrayList<String> PRE_READING, ADV, PRE_OFPK_RDNG, OFPK_ADV, PRE_PK_RDNG, PK_ADV, MTR_COND,METER_READING_ID;
		...
		METER_READING_ID = new ArrayList<String>(Arrays.asList(mrsEntryModel.METER_READING_ID.split(",")));
		for (int i = 0; i < CUST_ID.size(); i++) {
			...
			if (METER_READING_ID.isEmpty()) {
				METER_READING_ID.add(null);
			} else if (i == METER_READING_ID.size()) {
				METER_READING_ID.add(null);
			}
		}
		int OutPut = 0;
		OutPut = mainDao.updateMrsData(CUST_ID, BILL_CYCLE_CODE, PRE_READING, ADV, PRE_OFPK_RDNG, OFPK_ADV, PRE_PK_RDNG,
				PK_ADV, MTR_COND,METER_READING_ID, "");
		if (OutPut == 1) {
