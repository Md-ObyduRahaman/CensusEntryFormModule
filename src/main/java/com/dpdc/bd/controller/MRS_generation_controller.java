...
@Controller
public class MRS_generation_controller {
	...
	@Resource
	MRS_GenerationDao mrs_GenerationDao;
	
	@GetMapping("/MRS_GENERATION")
	public String billHome(@CookieValue(value = "user_name", defaultValue = "") String user_name, Model model,
		...
		return "MRS_Generation";
	}
	
	
	@PostMapping("/Result_MrsEntry")
	public String showBill(@CookieValue(value = "user_name", defaultValue = "") String user_name,
		...
		String outPut=mrs_GenerationDao.insertMRSEntry(addMeterModel);
		
		if (outPut!="1") {
			
			model.addAttribute("msg", "MRS Entry Generate Successfully");
			System.out.println("Hello msg");
		}
		else {
			
			model.addAttribute("msg", "MRS Entry not Generated! Please try again! ");
